	...
	#updateRecentItems(){
		...
		this.#db.getAll("media").then(data => {
			data.forEach(obj => {
				...
				// });
				ele.addEventListener('pointerdown' , e => this.#itemPointerDown(ele,this.#tabRecentEle));
				ele.addEventListener('pointermove' , e => this.#itemPointerMove(e));
				ele.addEventListener('pointerup' , e => this.#itemPointerUp(ele,obj,this.#tabRecentEle));
			...
			if(playlistId && typeof playlistId == "number"){
				...
				this.#db.getAll("media").then(data => {
					data.forEach(obj => {
						...
						/*
						ele.addEventListener('pointerdown' , e => this.#itemPointerDown(ele,this.#tabRecentEle));
						ele.addEventListener('pointerup' , e => this.#itemPointerUp(ele,this.#tabRecentEle));
						*/
	...
	#playVideo(obj,ele){
		...
		if(ele.dataset.playlist){
			if(this.#currentPlaylist !== ele.dataset.playlist){
				this.#updatePlayerListItems(ele.dataset.playlist);
		...
		*/
		...
		// this.#playerVideoEle = this.#playerEle.querySelector(".site-player-video video");
		// this.#playerItemsListEle = this.#playerEle.querySelector(".site-list-items");
	}
